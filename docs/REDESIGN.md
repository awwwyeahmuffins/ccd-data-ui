...
| **Trends** `trends.html` | "How did each precinct's Dem-vs-Rep balance MOVE from one cycle to the next — and does it matter in votes?" | Chair / field director / researcher | `metric, ya, yb, precinct (highlight), boundary` | Arrow map (Dem up-right / Rep down-left, length = net votes or points) + swing scatter + county headline with per-base vote changes + filterable per-precinct table | Pick metric (general composite / primary participation) and year pair, stack filters (direction, swing, closeness, votes, impact, vote base, precinct list), select a precinct anywhere (click, tap, arrow keys), sort the table | Mark/polygon/row → **My Precinct**; caveat banner → **How It Works** |
