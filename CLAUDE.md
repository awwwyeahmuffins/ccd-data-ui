...
- `domain/trends.js` — cross-year race-family normalization (`getRaceFamilyKey`) + margin deltas + the Trends page's two-party composite (`twoPartyMargin`, `selectCompositeRaces`, `buildPartisanIndex`, `buildSwingSeries`, `summarizeSwing`). NOTE the two margin conventions: `computePrecinctDeltas` measures a side against every other candidate (Lib/Grn count against it), while `twoPartyMargin` excludes third parties from both sides — the composite uses the latter
