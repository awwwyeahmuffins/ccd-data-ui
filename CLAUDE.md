...
**Statewide status:** all 254 Texas counties are selectable (`data/tx/counties.json` registry); **175 are live** with audited results (2022 or 2020 general + election-vintage TLC boundaries), including Harris, Dallas, Tarrant, Travis, El Paso, Denton, Collin (full archive, 2024+2026 sets). The other 79 are explicit PLACEHOLDERS — 78 held by the import gates (per-county reasons in `data/tx/AUDIT_REPORT.json`: audit discrepancies vs official sources, or boundary-join gaps) and Kinney (no published precinct data). NEVER fabricate data for a placeholder county, and never weaken a gate to flip one — fix the data. Pipeline: `data_processor/batch_import.py` (three hard gates: ETL, exact canvass audit vs MEDSL/VEST official datasets, 100% boundary join); single-county recipe in `docs/ADDING_FEATURES.md` Recipe F.
