...
function makeHandle(id) {
   ...
   * voter file). Resolves { [precinct]: { active, inactive, share } } or null
   * when not on file (the null is cached — absence is an answer, not a
   * failure). A blank inactive cell (small-cell suppression) stays null,
   * never zero.
   */
      ...
      try {
        const rows = await globalThis.d3.csv(
          ...
          (d) => ({
            ...
            inactive: d.inactive_voters === "" ? null : +d.inactive_voters,
            share: d.inactive_share === "" ? null : +d.inactive_share,
          })
        );
        const lookup = {};
        for (const r of rows) {
  ...
  /**
   * Canvass-coverage aggregates by precinct (profile extra — optional, N/A
   * when absent; generated by data_processor/build_canvass.py from a
   * canvassing workbook). Resolves { [precinct]: { demVoters, share,
   * canvassed } } or null when not on file (the null is cached — absence is an
   * answer, not a failure). A blank share cell stays null, never zero.
   */
  function loadCanvass() {
    return memoize(memo, "canvass", async () => {
      try {
        const rows = await globalThis.d3.csv(
          `${config.profileDir}/canvass.csv`,
          (d) => ({
            precinct: String(d.precinct),
            demVoters: d.dem_voters === "" ? null : +d.dem_voters,
            share: d.canvassed_share === "" ? null : +d.canvassed_share,
            canvassed: d.canvassed_voters === "" ? null : +d.canvassed_voters,
          })
        );
        const lookup = {};
        for (const r of rows) {
